...
class CVUSA_Noise4Random(torch.utils.data.Dataset):
    ...
    def __init__(self, mode = '', root = '/home/c3-0/parthpk/CVUSA/', query_root = '/home/ak362297/TransGeo2022/FINALCVUSANoiseSeverity4/',same_area=True, print_bool=False, polar = '', args=None, noiseName='Gaussian Blur', severity = 4): #CV-dataset
        super(CVUSA_Noise4Random, self).__init__()
    ...
    def __getitem__(self, index, debug=False):
        if self.mode == 'train':
            ...
            noise_type = random.choice(['Gaussian Noise', 'Brightness', 'Contrast', 'Gaussian Blur', 'Hue', 'Saturate'])
            #print('noisetype selected is', noise_type)
           	
            if noise_type == 'Gaussian Noise':	
                noisy_img_query = add_gaussian_noise(img_query, mean=0.0, std=self.severity)	
             #   print('Gaussian Noise', type(noisy_img_query))
        ...
        #   print(self.query_root + self.id_test_list[index][1])
            if noise_type == 'Gaussian Noise':	
                noisy_img_query = add_gaussian_noise(img_query, mean=0.0, std=self.severity)
 #               print('gaussian', type(noisy_img_query))	
