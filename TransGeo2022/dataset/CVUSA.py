# essentially the original CVUSA file made a few changes but is unused
...
class CVUSA(torch.utils.data.Dataset):
    ...
    def __init__(self, mode = '', root = '/home/c3-0/parthpk/CVUSA/', same_area=True, print_bool=False, polar = '',args=None): #CV-dataset
    ...
    def __getitem__(self, index, debug=False):
        if self.mode == 'train':
         ...
         #   img_query = Image.fromarray(np.uint8(img_query))  # Convert NumPy array back to PIL Image
            ...
            img_reference = Image.open(self.root + self.id_test_list[index][0]).convert('RGB')
 #          print("full path" + self.root + self.id_test_list[index][0])
            img_reference = self.transform_reference(img_reference)
            if self.args.crop:
                print('indexxx')
                print(index)
            ...
            idx = index % len(self.id_idx_list)
            print("path" + self.root + self.id_list[idx][1])
            # /home/c3-0/parthpk/CVUSA/
            img_query = Image.open(self.root + self.id_test_list[index][1]).convert('RGB')
            img_query = self.transform_query(img_query)
