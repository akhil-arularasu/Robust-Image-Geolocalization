...
class TransGeo(nn.Module):
    ...
    def forward(self, im_q_clean, im_q_noisy, im_k, delta=None, atten=None, indexes=None):
        if atten is not None:
            return self.query_net(im_q_clean), self.query_net(im_q_noisy), self.reference_net(x=im_k, atten=atten)
        else:
            return self.query_net(im_q_clean), self.query_net(im_q_noisy), self.reference_net(x=im_k, indexes=indexes)
