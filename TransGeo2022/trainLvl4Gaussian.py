...
import numpy as np
from dataset.CVUSA_Noise4Gaussian import CVUSA_Noise4Gaussian
...
def custom_collate(batch):
    ...
    atten_tensors = [tensor for tensor in atten if isinstance(tensor, torch.Tensor)]
    if atten_tensors:
        atten = torch.stack(atten_tensors, dim=0)
    else:
        atten = torch.tensor(atten)
    return images, indexes, atten
'''
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.dataset == 'vigor':
        ...
        dataset = CVUSA_Noise4Gaussian
        mining_sampler = DistributedMiningSampler
    elif args.dataset == 'cvact':
        dataset = CVACT
        mining_sampler = DistributedMiningSampler

    noise_list = ['Gaussian Noise', 'Shot Noise', 'Impulse Noise', 'Defocus Blur', 'Glass Blur', 'Motion Blur', 'Zoom Blur', 'Snow', 'Fog', 'Brightness', 'Contrast', 'Elastic', 'Pixelate', 'JPEG', 'Speckle Noise', 'Gaussian Blur', 'Spatter', 'Saturate']
    noiseName = noise_list[0]

    train_dataset = dataset(mode='train', print_bool=True, same_area=(not args.cross),args=args)
    train_scan_dataset = dataset(mode='scan_train' if args.dataset == 'vigor' else 'train', print_bool=True, same_area=(not args.cross), args=args, noiseName=noiseName)
    ...
    val_reference_loader = torch.utils.data.DataLoader(
        ...
        num_workers=8, pin_memory=True, drop_last=False) # 80, 128
    # collate_fn=custom_collate
    if args.evaluate:
        if not args.multiprocessing_distributed or args.gpu == 0:
            print('went into aaaaaaargs.evaluate')
            validate(val_query_loader, val_reference_loader, model, args, noiseName)
        return
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if not args.multiprocessing_distributed or args.gpu == 0:
            acc1 = validate(val_query_loader, val_reference_loader, model, args, noiseName)
...
def train(train_loader, model, criterion, optimizer, epoch, args, train_sampler=None):
    ...
    for i, (images_q, images_k, indexes, _, delta, atten) in enumerate(train_loader):
        ...
        if args.gpu is not None:
            images_q_clean = images_q[0].cuda(args.gpu, non_blocking=True)
            images_q_noisy = images_q[1].cuda(args.gpu, non_blocking=True)
            images_k = images_k.cuda(args.gpu, non_blocking=True)
            indexes = indexes.cuda(args.gpu, non_blocking=True)
        else:
            images_q_clean = images_q[0]
            images_q_noisy = images_q[1]
            images_k = images_k
            indexes = indexes

        # compute output
        if args.crop:
            embed_q_clean, embed_q_noisy, embed_k = model(im_q_clean=images_q_clean, im_q_noisy=images_q_noisy, im_k=images_k, delta=delta, atten=atten)
        else:
            embed_q_clean, embed_q_noisy, embed_k = model(im_q_clean=images_q_clean, im_q_noisy=images_q_noisy, im_k=images_k, delta=delta)
	
        loss, mean_p, mean_n = criterion(embed_q_clean, embed_q_noisy, embed_k)

        if args.mining:
            #print('shape of indexes:')
            #print(indexes.shape)
            train_sampler.update(concat_all_gather(embed_k).detach().cpu().numpy(),concat_all_gather(embed_q_clean).detach().cpu().numpy(),concat_all_gather(indexes).detach().cpu().numpy())
        losses.update(loss.item(), images_q_clean.size(0) + images_q_noisy.size(0))
        mean_ps.update(mean_p, images_q_clean.size(0) + images_q_noisy.size(0))
        mean_ns.update(mean_n, images_q_clean.size(0) + images_q_noisy.size(0))
...
def validate(val_query_loader, val_reference_loader, model, args, noise_name):
    batch_time = AverageMeter('Time', ':6.3f')
    ...
    with torch.no_grad():
        ...
        [top1, top5] = accuracy(query_features, reference_features, query_labels.astype(int), noise_name)
...
class AverageMeter(object):
    ...
    def update(self, val, n=1):
        ...
        if self.name == 'Data' and self.val > 5.0:
            print("in update", self.name, self.val, n, self.sum, self.count, self.avg)
...
def accuracy(query_features, reference_features, query_labels, noise_name, topk=[1,5,10]):
    ...
    with open('trainNoiselvl4GaussianEvaluationValues.txt', 'a') as file:
...
if __name__ == '__main__':
    ...
    '''
    # uncomment when you need to create CSV files inside CVUSANoise_CSVs directory
    def create_csv_files_for_noises(noise_list):
        base_csv_file = '/home/c3-0/parthpk/CVUSA/splits/val-19zl.csv'
        output_dir = '/home/ak362297/TransGeo2022/CVUSANoise_CSVs/'  # Replace with the desired output directory path

        # Read the base CSV file
        with open(base_csv_file, 'r') as file:
            csv_data = list(csv.reader(file))

        # Iterate over the noise list
        for noise in noise_list:
            # Create a new CSV file name based on the noise name
            noise_csv_file = os.path.join(output_dir, f'{noise}.csv')

            # Create a duplicate of the base CSV data for the current noise
            noise_csv_data = csv_data.copy()

            # Modify the relevant column in the CSV data with the noise name
            for row in noise_csv_data:
                row[1] = noise + '/' + row[1].split('/')[-1]

            # Write the modified CSV data to the new CSV file
            with open(noise_csv_file, 'w', newline='') as file:
                writer = csv.writer(file)
                writer.writerows(noise_csv_data)

            print(f"CSV file created for noise: {noise} at {noise_csv_file}")
    '''